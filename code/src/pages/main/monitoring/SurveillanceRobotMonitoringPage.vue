...
const selectedMapIds = ref<number[]>([3])
...
onMounted(async () => {
    ...
    unsubscribeSim = simulationService.subscribe((updatedRobots) => {
        ...
        const filtered = updatedRobots.filter(r => r.robotType === 'SURVEILLANCE')
...
const openGlobalEmergency = async () => {
    try {
        await ElMessageBox.confirm(
            '시설 내 감시 로봇에 일괄 E-Stop(비상정지)을 집행하시겠습니까?',
            '감시 로봇 일괄 E-STOP 비상집행',
            {
                type: 'error',
                confirmButtonText: '감시 로봇 E-Stop 일괄 집행',
                cancelButtonText: '취소',
                customClass: 'estop-confirm-dialog',
            },
        )
        robots.value.forEach(r => simulationService.applyCommand(r.id, 'E_STOP'))
        ElMessage.error('감시 로봇 일괄 E-STOP 비상정지가 전송되었습니다.')
    } catch {
        // Cancelled
    }
}
