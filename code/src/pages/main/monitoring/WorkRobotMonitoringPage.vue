...
const kind: TypedMonitoringKind = 'WORK'

const snapshot = ref<IntegratedMonitoringSnapshot | null>(null)
const loading = ref(true)
const selectedMapIds = ref<number[]>([2])
...
onMounted(async () => {
    ...
    unsubscribeSim = simulationService.subscribe((updatedRobots) => {
        ...
        const filtered = updatedRobots.filter(r => r.robotType === 'WORK')
        snapshot.value.robots = filtered
        snapshot.value.counts = {
            total: filtered.length,
            online: filtered.filter(r => r.communicationStatus === 'ONLINE').length,
            stale: filtered.filter(r => r.communicationStatus === 'STALE').length,
            offline: filtered.filter(r => r.communicationStatus === 'OFFLINE').length,
            work: filtered.filter(r => r.robotType === 'WORK').length,
            surveillance: filtered.filter(r => r.robotType === 'SURVEILLANCE').length,
        }
...
const openGlobalEmergency = async () => {
    try {
        await ElMessageBox.confirm(
            '시설 내 작업 로봇에 일괄 E-Stop(비상정지)을 집행하시겠습니까?',
            '작업 로봇 일괄 E-STOP 비상집행',
            {
                type: 'error',
                confirmButtonText: '작업 로봇 E-Stop 일괄 집행',
                cancelButtonText: '취소',
                customClass: 'estop-confirm-dialog',
            },
        )
        robots.value.forEach(r => simulationService.applyCommand(r.id, 'E_STOP'))
        ElMessage.error('작업 로봇 일괄 E-STOP 비상정지가 전송되었습니다.')
    } catch {
        // Cancelled
    }
}
